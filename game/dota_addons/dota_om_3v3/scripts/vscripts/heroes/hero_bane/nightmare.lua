...
function NightmareDamage( keys )
	...
	if unit_health < damage + 1 then
		...
		target:SetHealth(unit_health - damage)
	end
	...
	for i,unit in pairs(targets) do
		if unit ~= target then
			...
			if unit_health < damage + 1 then
				...
				-- Otherwise just set the health to be lower
				unit:SetHealth(unit_health - damage)
			end
		end
	end
end
...
function NightmareCasterCheck( keys )
	...
	if bane_nightmare_target == nil then
		bane_nightmare_target = {}
	end
	bane_nightmare_target[playerid] = target
...
function NightmareCasterEnd( keys )
	...
	local ability = keys.ability
	local playerid = caster:GetPlayerOwnerID()
	bane_nightmare_target[playerid]:RemoveModifierByName("modifier_nightmare_om")
	bane_nightmare_target[playerid]:RemoveModifierByName("modifier_bane_nightmare_om_invul")

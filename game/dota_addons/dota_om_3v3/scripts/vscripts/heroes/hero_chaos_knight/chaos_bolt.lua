...
function ChaosBolt( keys )
	...
	local range_min = ability:GetLevelSpecialValueFor("range_min", ability_level) 
	local range_max = ability:GetLevelSpecialValueFor("range_max", ability_level)
	...
	local range = range_min + (range_max - range_min) * (1 - random)
